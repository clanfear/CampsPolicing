...
camping_beats_half_2012_2019 <- general %>% 
  ...
  select(year, call_half, geometry) %>%
  ...
  ungroup() %>%
  complete(beat, year = 2012:2019, call_half = 1:2, fill = list(n_camping_complaints =0)) %>%
  filter(!(year==2019 & call_half==2)) %>%
  arrange(beat, year, call_half)

save(camping_beats_half_2012_2019, file = "./data/derived/camping_beats_half_2012_2019.RData")
