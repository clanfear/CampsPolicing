...
fenegbin(complaints ~ l(property, 1) + l(violent, 1) + l(complaints, 1) | tract + date, 
...
panels_bg <- comp_prop_panel_bg_month %>% 
  mutate(date = as.numeric(as.factor(date))) %>%
  group_by(blockgroup) %>%
  filter(!all(complaints==0)) %>%
  ungroup() %>%
  {list(
    "pois_fe_lag_viol" = fepois(complaints ~ l(property, 1) + l(violent, 1) + l(complaints, 1) | blockgroup + date, 
                                panel.id = ~ blockgroup + date, data = ., vcov = "DK"),
    "pois_fe_nolag_viol" = fepois(complaints ~ l(property) + l(violent) | blockgroup + date, 
                                  panel.id = ~ blockgroup + date, data = ., vcov = "DK"),
    "pois_trend_lag_viol" = fepois(complaints ~ l(property) + l(violent) + l(complaints) | blockgroup[date], 
                                   panel.id = ~ blockgroup + date, data = . , vcov = "DK"),
    "pois_trend_nolag_viol" =fepois(complaints ~ l(property) + l(violent) | blockgroup[date], 
                                    panel.id = ~ blockgroup + date, data = ., vcov = "DK"),
    "lm_fe_lag_viol" = feglm(complaints ~ l(property) + l(violent) + l(complaints) | blockgroup + date, 
                             panel.id = ~ blockgroup + date, data = ., vcov = "DK"),
    "lm_fe_nolag_viol" = feglm(complaints ~ l(property) + l(violent) | blockgroup + date, 
                               panel.id = ~ blockgroup + date, data = ., vcov = "DK"),
    "lm_trend_lag_viol" = feglm(complaints ~ l(property) + l(violent) + l(complaints) | blockgroup[date], 
                                panel.id = ~ blockgroup + date, data = ., vcov = "DK"),
    "lm_trend_nolag_viol" = feglm(complaints ~ l(property) + l(violent) | blockgroup[date], 
                                  panel.id = ~ blockgroup + date, data = ., vcov = "DK"),
    "pois_fe_lag_noviol" = fepois(complaints ~ l(property, 1)  + l(complaints, 1) | blockgroup + date, 
                                  panel.id = ~ blockgroup + date, data = ., vcov = "DK"),
    "pois_fe_nolag_noviol" = fepois(complaints ~ l(property)  | blockgroup + date, 
                                    panel.id = ~ blockgroup + date, data = ., vcov = "DK"),
    "pois_trend_lag_noviol" = fepois(complaints ~ l(property) + l(complaints) | blockgroup[date], 
                                     panel.id = ~ blockgroup + date, data = . , vcov = "DK"),
    "pois_trend_nolag_noviol" =fepois(complaints ~ l(property)  | blockgroup[date], 
                                      panel.id = ~ blockgroup + date, data = ., vcov = "DK"),
    "lm_fe_lag_noviol" = feglm(complaints ~ l(property)  + l(complaints) | blockgroup + date, 
                               panel.id = ~ blockgroup + date, data = ., vcov = "DK"),
    "lm_fe_nolag_noviol" = feglm(complaints ~ l(property) | blockgroup + date, 
                                 panel.id = ~ blockgroup + date, data = ., vcov = "DK"),
    "lm_trend_lag_noviol" = feglm(complaints ~ l(property)  + l(complaints) | blockgroup[date], 
                                  panel.id = ~ blockgroup + date, data = ., vcov = "DK"),
    "lm_trend_nolag_noviol" = feglm(complaints ~ l(property)  | blockgroup[date], 
                                    panel.id = ~ blockgroup + date, data = ., vcov = "DK"))}

map_df(panels_bg, ~ tidy(.x, conf.int=TRUE), .id = "model") %>%
  separate(model, into = c("link", "fe", "lag", "violence"), sep = "_") %>%
  mutate(model = paste(fe, lag, violence, sep = "_")) %>%
  filter(str_detect(term, "property")) %>%
  ggplot(aes(x = estimate, y = model)) + 
...
# Panel
hist(comp_prop_cross_tract$complaints)
