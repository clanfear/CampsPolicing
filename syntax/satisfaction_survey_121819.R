...
spd_cs_survey <- survey_1_42 %>%
  select(id = ID,
         ...
         q11_fear_overall = Q11, # Fear of crime 11 (1)
         q12_fear_incident_change = Q12,
         q13_fear_polresponse_change = Q13,
         q14_dept_satisfaction_change = Q14,
         q15_fear_neighb_walk_day = Q15, # Fear of crime 15 (2)
         q16_fear_neighb_walk_night = Q16, # Fear of crime 16 (3)
...
fear_of_crime_df <- spd_cs_survey %>%
  select(unique_id,
         q11_fear_overall,
         q15_fear_neighb_walk_day,
         q16_fear_neighb_walk_night) %>% 
  mutate_at(vars(-unique_id), ~ ifelse(. %in% c(7,9), NA, .)) %>%
  mutate_at(vars(-unique_id), ~ standardize(.)) %>%
  pivot_longer(-unique_id, names_to = "measure", values_to = "value") %>%
  group_by(unique_id) %>%
