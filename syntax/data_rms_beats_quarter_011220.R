...
spd_rms_geo_beat_quarter <- spd_rms_geo %>% 
  st_transform(3690) %>%
  ...
  ungroup() %>%
  complete(beat, year = 2008:2019, call_quarter = 1:4, 
...
#spd_rms_rates_beat_quarter <- 
temp_rms <- spd_rms_geo_beat_quarter %>%
  inner_join(king_pop_2008_2019_aw, by = c("beat","year")) %>%
  mutate(population_int = ifelse(call_quarter != 1, NA, population)) %>%
  group_by(beat) %>%
  mutate(population_int = zoo::na.approx(population_int, maxgap = 3, rule =2)) %>%
  ungroup()

spd_rms_rates_beat_quarter <- temp_rms %>%
  mutate(population_int = 
           ifelse(year==2019 & call_quarter==2, 
                  predict(lm(population ~ year*beat, data = temp_rms), newdata = temp_rms %>% mutate(year = year + (call_quarter-1)*.25)), 
                  population_int)) %>%
  mutate_at(vars(starts_with("offense")), list(rate = ~./population, rate_int = ~./population_int))

save(spd_rms_rates_beat_quarter, file = "./data/derived/spd_rms_rates_beat_quarter.RData")
