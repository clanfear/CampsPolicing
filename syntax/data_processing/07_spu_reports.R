...
unauthorized_camping_complaints_monthly_tract <- unauthorized_camping_complaints %>%
  ...
  select(tract, year, month, complaints, fifi_complaints) %>%
  group_by(tract, year, month) %>%
  summarize_all(~sum(.)) %>%
  ungroup() %>%
  complete(tract = seattle_tract_boundaries$tract, nesting(year, month), fill = list(complaints = 0, fifi_complaints = 0)) %>%
...
unauthorized_camping_complaints_monthly_bg <- unauthorized_camping_complaints %>%
  ...
  mutate(complaints = 1,
         fifi_complaints = ifelse(method_received == "Find It Fix It Apps", 1 ,0)) %>%
  select(blockgroup, year, month, complaints, fifi_complaints) %>%
  group_by(blockgroup, year, month) %>%
  summarize_all(~sum(.)) %>%
  ungroup() %>%
  complete(blockgroup = seattle_bg_boundaries$blockgroup, nesting(year, month), fill = list(complaints = 0, fifi_complaints = 0)) %>%
  mutate(date = ym(paste(year, month, sep = "-")))%>%
  select(-year, -month)
